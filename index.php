...
<?php
    ...
    $printerData = $Conn->query(" SELECT `idprinter`, `namaprinter` FROM tb_printer ");
...
<body>
    ...
    <div class="container">
        ...
        <?= ($_SESSION['user'] != "admin") ? '<button id="adduser" class="button" style="margin-top:30px; padding:10px" onclick="showAddUser();">Add New Data</button>' : '' ?>
                ...
                <tr class="active-row">
                    ...
                    <td>
                        <div class="btn-group">
                            ...
                            <?= ( $_SESSION['user'] == "admin" ) ? ( ($hasil['userid2'] == 0) ? '<button> <a style="color:white;" href="process/konfirmasi.php?id='. $hasil['idtransaksi'] .'">Konfirmasi</a> </button>' : 'Done' ) : '
                                <div class="btn-group">
                                    ...
                                    <button> <a style="color:white;" href="process/delete.php?id='. $hasil['idtransaksi'] .'">Delete</a> </button>
                                </div>
                            ' ?>
                        </div>
                    </td>
        ...
        <form class="form-modal" action="process/add.php" method="post" autocomplete="off">
            ...
            <select name="namaprinter">
                <?php foreach( $printerData as $printer ) : ?>
                    <option value="<?=$printer['idprinter']?>"><?=$printer['namaprinter']?></option>
                <?php endforeach;?>
            </select>
